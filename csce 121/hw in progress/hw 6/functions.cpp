...
void removeVerticalSeam(Pixel **image, int& width, int height, int *verticalSeam)
{
  ...
  for(int ThisCol = 1 ; ThisCol < width ; ThisCol++)
  {
    for (int ThisRow = 1 ; ThisRow < height ; ThisRow++)
    {
      getVerticalSeam( image , ThisCol , width , height , verticalSeam );
      if (verticalSeam[ThisRow] == ThisCol)
      {
        verticalSeam.pop// ddelete at position array
        width--;
      }
    }
  }

  //compare the seams and remove the uneeded parts
}
