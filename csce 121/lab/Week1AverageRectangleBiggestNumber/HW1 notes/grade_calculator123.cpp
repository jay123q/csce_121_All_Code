...
int main() {
    ...
    double LabworkGrades;
    double EngagementGrades; 
    //double Storage;
    ...
    while (!line.empty()) {
        ...
        if (category == "exam") {
            ...
            ExamGrades += score; // Student Notes: set the Final equal to the score
            DividenExam++;
            ...
            LabworkGrades += score*100; // Student notes adding each score to the appropriate grade
    ...
    double hw_average = 0;
    double lw_average = 0;
    double engagement = EngagementGrades; //this is unneeded mmimir wants total engagement

 // Student Notes Ideally I want ed to do this step in the if statements, but now I will run the math for the average

 // if there is no input score creates a nubmer that is -1E-142 I will make a for loop to correct any errors


     // Student Notes hhaving final as a seperate variables was useless sincee its the same average
    if (ExamGrades>0){
        if (ExamGrades > 100){
            ExamGrades = 100;
        }
        exam_average = (ExamGrades) / DividenExam;
    }
    ...
    if (HomeworkGrades>.1){
        ...
        if (HomeworkGrades > 100){
            HomeworkGrades = 100;
        }        
    }
    if (LabworkGrades>.1){

        if (LabworkGrades > 100){
            LabworkGrades = 100;
        }    
    ...
    double ExamWeight; // Student Notes intialize variables orderly
    ...
    char final_letter_grade = 'X'; // Student Notes I will be using && statments and including edge cases by uisng 89.5 etc
